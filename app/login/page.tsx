...
import { CinematicLogin } from "@/components/auth/CinematicLogin";
...
/** The email/password form — shared by the web and app layouts. */
function EmailLoginForm({ error, next }: { error?: string; next: string }) {
  return (
    <>
      {error && (
        <div
...
export default async function LoginPage({
  ...
  // The Capacitor shells (Android/iOS) append "MLDNative" to their WebView
  // user agent — only they get the cinematic full-screen login. Every
  // browser, including mobile browsers, keeps the standard web layout.
  const userAgent = (await headers()).get("user-agent") ?? "";
  if (userAgent.includes("MLDNative")) {
    return (
      <CinematicLogin next={next} startOnEmail={Boolean(error)}>
        <div className="glass-noir rounded-[2rem] p-6 sm:p-7">
          <EmailLoginForm error={error} next={next} />
        </div>
